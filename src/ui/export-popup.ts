...
class ExportPopup extends Container {
    ...
    constructor(events: Events, args = {}) {
        ...
        const reset = (exportType: ExportType, splatNames: string[], hasPoses: boolean) => {
            const allRows = [
                viewerTypeRow, startRow, animationRow, loopRow, colorRow, fovRow, compressRow, splatsRow, bandsRow, iterationsRow, filenameRow
            ];
            ...
            const activeRows = {
                ...
                viewer: [viewerTypeRow, startRow, animationRow, loopRow, colorRow, fovRow, splatsRow, bandsRow, filenameRow],
                config: [startRow, animationRow, loopRow, colorRow, fovRow, filenameRow]
            }[exportType];
        ...
        this.show = (exportType: ExportType, splatNames: string[], showFilenameEdit: boolean) => {
            ...
            const getSelectedSplatIdx = () => {
                return splatsSelect.value === 'all' ? 'all' : Number(splatsSelect.value);
            };

            const assemblePlyOptions = () : SceneExportOptions => {
                return {
                    filename: filenameEntry.value,
                    splatIdx: getSelectedSplatIdx(),
                    serializeSettings: {
            ...
            const assembleViewerOptions = () : SceneExportOptions => {
                const experienceSettings = mergeExperienceSettings(
                    events.invoke('settings.raw') as Record<string, any> | undefined,
